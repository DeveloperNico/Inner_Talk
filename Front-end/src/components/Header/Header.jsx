import { Leaf, House, NotebookPen, LogIn, LayoutDashboard, Stethoscope, SmileIcon, CalendarDays, Link2 } from 'lucide-react';
...
const PACIENTE_LINKS = [
    ...
    { to: '/vinculo', icon: Link2, label: 'Meu Psicólogo' },
];
    ...
    return (
        <header className={`fixed top-0 left-0 right-0 z-[150] bg-[#EDFDF8] items-center justify-between py-4 px-20 flex ${scrolled ? 'shadow-md' : 'shadow-none'}`}>
            <section className="flex items-center gap-5">
                ...
                {links.length > 0 && (
                    <>
                        <span className="h-8 w-px bg-[#509486] self-center" />
                        <nav>
                                ...
                                {links.map(({ to, icon: Icon, label }) => {
                                    ...
                                    return (
                                        <li key={label}>
                                            <Link
                                                to={to}
                                                className={`flex gap-2 items-center font-secondary transition-colors px-3 py-2 rounded-full ${
                                                    isActive
